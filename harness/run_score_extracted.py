"""Run Observathon scorer while bypassing the PyInstaller bootloader.

Expected layout after the public/private scorer is released:
    bin/<phase>/observathon-score/observathon-score.exe
    bin/<phase>/observathon-score/_internal/

Usage:
    python harness/run_score_extracted.py --phase public --run run_output.json \
        --findings solution/findings.json --team DangHuuNghia --out score.json
"""
...
FULL = ROOT / ".pyi-score-full"
CODE = ROOT / ".pyi-score-code"
...
def main() -> None:
    ...
    if not exe.exists() or not internal.exists():
        raise SystemExit(
            f"missing scorer onedir files under {exe.parent}; wait for the {phase} score binary, then retry"
        )
    ...
    from observathon_score.cli import main as score_main
